import { CheckCircle2, Circle, BookOpen, CalendarClock } from "lucide-react";
...
const TodayTasks = () => {
  ...
  const tasks = useMemo(() => {
    const result: { id: string; title: string; subject: string; revisionLabel: string }[] = [];
    for (const sub of subjects) {
      for (const ch of sub.chapters) {
        if (ch.is_completed && ch.completed_date) {
          const dueIdx = getDueRevisions(ch.completed_date, ch.revisions_completed, today);
          if (dueIdx !== null) {
            result.push({
              id: ch.id,
